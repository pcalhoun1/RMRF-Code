...
singleSim_RFvsGLM <- function(x, model=LETTERS[1:4], intercept, beta, sigma, rho){
  ...
  source('C:/Users/pcalh/Desktop/RMRF Code/R Functions/RF functions 20JAN19.R')
  #source('RF functions 20JAN19.R')
  
  model <- match.arg(model, LETTERS[1:4])
  
  N <- 400   #Number of patients
  clsize <- 4   #Number of nights
  x1 <- rnorm(clsize*N, mean=0, sd=1)
  x2 <- rnorm(clsize*N, mean=0, sd=1)
  x3 <- rnorm(clsize*N, mean=0, sd=1)
  x4 <- rnorm(clsize*N, mean=0, sd=1)
  x5 <- rnorm(clsize*N, mean=0, sd=1)
  x1_Sq <- x1^2
  x2_Sq <- x2^2

  w <- rep(rnorm(N, mean=0, sd=sigma), each=clsize)
  if (model=="A") {prob <- 1/(1+exp(-(intercept + beta*x1 + beta*x2 + w)))
  } else if (model=="B") {prob <- 1/(1+exp(-(intercept + beta*x1_Sq + beta*x2_Sq + w)))
  } else if (model=="C") {prob <- 1/(1+exp(-(intercept + beta*x1*x2 + w)))
  } else if (model=="D") {prob <- 1/(1+exp(-(intercept + beta*x1_Sq*x2_Sq + w)))
  ...
  rfIndep <- growRF(ntrees=500, formula=Class ~ x1 + x2 + x3 + x4 + x5,
                    ...
                    mtry=2, nsplit=50, nsplit.random=FALSE, minsplit=3, minbucket=1, maxdepth=2, sampleMethod="subsample")
  
  rfDep <- growRF(ntrees=500, formula=Class ~ x1 + x2 + x3 + x4 + x5 | id,
                  data=dat, search="exhaustive", method="class", split="gini",
                  mtry=2, nsplit=50, nsplit.random=FALSE, minsplit=3, minbucket=1, maxdepth=2, sampleMethod="subsampleByID", corstr="exchangeable")
  
  erIndep <- growRF(ntrees=500, formula=Class ~ x1 + x2 + x3 + x4 + x5,
                    data=dat, search="exhaustive", method="class", split="gini",
                    mtry=2, nsplit=1, nsplit.random=TRUE, minsplit=3, minbucket=1, maxdepth=2, sampleMethod="subsample")
  
  erDep <- growRF(ntrees=500, formula=Class ~ x1 + x2 + x3 + x4 + x5 | id,
                  data=dat, search="exhaustive", method="class", split="gini",
                  mtry=2, nsplit=1, nsplit.random=TRUE, minsplit=3, minbucket=1, maxdepth=2, sampleMethod="subsampleByID", corstr="exchangeable")
...
# Function takes results and returns rank of X1 and X2
vi_max <- function(simData){
  data.frame(lapply(names(simData), function(x){setNames(data.frame(simData[[x]][,c(1,2)]), paste0(x, c("1", "2")))}))
}
